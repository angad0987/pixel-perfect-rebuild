...
function OrderPage() {
      ...
      <div className="min-h-screen bg-section-muted flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center px-4">
          <div className="text-center">
            ...
            <Link
              to="/account"
              className="inline-flex items-center gap-2 mt-4 text-brand-accent text-sm font-medium"
            >
  ...
  const stepsToShow = isCancelled ? ORDER_STEPS.slice(0, 1) : ORDER_STEPS;
  const stepIdx = stepsToShow.indexOf(order.status);
  ...
  return (
    <div className="min-h-screen bg-section-muted flex flex-col">
      ...
      <main className="flex-1 mx-auto max-w-4xl w-full px-4 py-8">
        <Link
          to="/account"
          className="text-sm text-muted-foreground inline-flex items-center gap-1 hover:text-brand-accent"
        >
        ...
        <div className="bg-white rounded-2xl border border-border/60 p-6 mt-4">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <span
              className={`text-xs px-3 py-1.5 rounded-full font-semibold ${STATUS_BADGE[order.status] || "bg-gray-100 text-gray-700"}`}
            >
              {order.status}
            </span>
          </div>
          ...
          <div className="flex gap-4 mt-6 pb-6 border-b border-border">
            <img
              ...
              className="h-24 w-24 rounded-xl object-cover bg-section-muted"
            />
          ...
          {isCancelled ? (
            <div className="mt-6 p-4 bg-red-50 text-red-700 rounded-xl text-sm font-medium flex items-center gap-2">
              <XCircle className="h-5 w-5 shrink-0" />
              This order has been cancelled.
            </div>
          ) : (
            <>
              <h2 className="font-display text-lg font-bold text-brand mt-6 mb-4">
                Tracking Timeline
              </h2>
                  ...
                  return (
                    <li key={s} className="flex gap-4 pb-6 last:pb-0 relative">
                      {i < stepsToShow.length - 1 && (
                        <span
                          className={`absolute left-5 top-10 bottom-0 w-0.5 ${i < stepIdx ? "bg-brand-accent" : "bg-section-muted"}`}
                        />
                      )}
                      <div
                        className={`relative h-10 w-10 rounded-full flex items-center justify-center shrink-0 ${done ? "bg-brand-accent text-white" : "bg-section-muted text-muted-foreground"} ${current ? "ring-4 ring-brand-accent/20" : ""}`}
                      >
                        <Icon className="h-5 w-5" />
                      </div>
                      <div className="pt-1.5">
                        <div
                          className={`text-sm font-semibold ${done ? "text-brand" : "text-muted-foreground"}`}
                        >
                          {s}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {entry ? new Date(entry.at).toLocaleString() : "Pending"}
                        </div>
                      </div>
                    </li>
                  );
          ...
          <div className="mt-6 p-4 bg-section-muted rounded-xl flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                Chat with our team on WhatsApp for instant updates.
              </div>
            </div>
            <a
              ...
              className="inline-flex items-center gap-2 bg-[#25D366] hover:bg-[#1ebe5d] text-white font-semibold px-4 py-2.5 rounded-xl"
            >
